...
- Demande d'autorisation Dropbox en accès hors-ligne (`token_access_type=offline`, jeton de rafraîchissement) et limitée à quatre portées : `account_info.read`, `files.metadata.read`, `files.content.write`, `files.content.read` — chacune justifiée dans l'ADR et dans la documentation utilisateur. Refs #10
